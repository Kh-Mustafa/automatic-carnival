...
// should return a `bool`.
    ...
    bool permission_check(Action takenAction, AccountStatus actor) {
        if ((takenAction == Action::read)
            || (takenAction == Action::write && (actor == AccountStatus::user || actor == AccountStatus::troll))
            || (actor == AccountStatus::mod)) {
            return true;
        } else {
            return false;
        }
